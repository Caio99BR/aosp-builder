...
download_ccache()
{
  ...
  aria2c "${builder_ccache_url}" -x50 -s50 || { bot_send "File not found..."; exit 1; }
...
builder_no_sanity="${buildsh_working_dir}/.repo/repo/repo"
...
python ${builder_no_sanity} init -q --no-repo-verify --depth=1 -u ${rom_manifest} -b ${rom_manifest_branch} -g default,-device,-mips,-darwin,-notdefault
...
python ${builder_no_sanity} sync -c --no-clone-bundle --no-tags --optimized-fetch --prune --force-sync -j 8 || python ${builder_no_sanity} sync -c --no-clone-bundle --no-tags --optimized-fetch --prune --force-sync -j 8
